...
created: 2026-04-12
description: "Learn how Karpathy's AutoResearch runs 100+ ML experiments overnight on a single GPU. Covers the three-file architecture, ratchet loop, results, and limitations."
tags:
  - "clippings"
---
Andrej Karpathy's AutoResearch is an [open-source tool](https://github.com/karpathy/autoresearch) that runs ML experiments in a loop, keeping only the changes that beat the current best result. You describe research directions in a markdown file, point an AI coding agent at the repo, and walk away. By morning, you have a git history of validated improvements and a log of everything the agent tried.
...
This is not [hyperparameter tuning](https://www.datacamp.com/tutorial/parameter-optimization-machine-learning-models). Tools like Optuna or Ray Tune search a predefined parameter space. AutoResearch gives the agent freedom to modify arbitrary code. The search space is whatever the LLM can think of, and that makes it a different category of tool from anything currently available.

[AutoML and NAS (Neural Architecture Search) frameworks](https://www.datacamp.com/blog/best-automl-frameworks) search over architectures or hyperparameters using structured algorithms, precise but constrained to their defined search space.

AlphaEvolve (Google DeepMind) goes further with an evolutionary approach and Gemini models for algorithm discovery, but it's closed-source and not accessible to most teams. General-purpose [coding agents](https://www.datacamp.com/tutorial/claude-code) like SWE-Agent, OpenHands, and Aider can write arbitrary code but aren't built for the experiment-evaluate-keep/revert cycle that ML research actually requires.
...
![Comparison of autoresearch against AlphaEvolve, AutoML, NAS, and SWE-Agent](https://media.datacamp.com/cms/34be3e00305b577e2279af3ba7ea5f5f.png "Comparison of autoresearch against AlphaEvolve, AutoML, NAS, and SWE-Agent")
...
![Progression from vibe coding to agentic engineering to independent research](https://media.datacamp.com/cms/image3-1a16b6.png)
...
![Three-file architecture showing file roles, ownership, and data flow](https://media.datacamp.com/cms/image2-67a0af.png)
...
![Ratchet loop flowchart showing the experiment cycle](https://media.datacamp.com/cms/356f194e5b09c62df9455e4f765ed5bd.png "Ratchet loop flowchart showing the experiment cycle")
...
cd autoresearch
uv sync
uv run prepare.pyPowered By Was this AI assistant helpful?
```
...
| --- | --- | --- | --- |
| Initial overnight (single GPU) | 83 | 15 | val\_bpb: 1.000 → 0.975 |
...
| Community session | 126 | \- | val\_bpb: 0.9979 → 0.9697 |
...
![Val BPB staircase showing incremental improvements across experiments](https://media.datacamp.com/cms/1f21ff9bb53c41be1c69a20c745871ca.png "Val BPB staircase showing incremental improvements across experiments")
...
[GitHub Issue #22](https://github.com/karpathy/autoresearch/issues/22) captures the structural problem. A user observed that the agent cycles through minor variations of whatever worked last, stuck in a local search pattern.
...
Karpathy acknowledged a related problem on [Hacker News](https://news.ycombinator.com/item?id=47291123): the agent feels "cagy and scared" on open-ended problems.

He attributes this to [RLHF](https://www.datacamp.com/courses/reinforcement-learning-from-human-feedback-rlhf) (Reinforcement Learning from Human Feedback) training, which rewards safe, conservative outputs over bold experimentation. The agent is capable of proposing creative changes but built to play it safe.
...
The three-file contract (immutable evaluator, agent-modifiable implementation, human-authored direction) transfers beyond [LLM training](https://www.datacamp.com/tutorial/fine-tuning-large-language-models) to any domain where you can define an automatic scoring function.
...
Karpathy himself runs a "bigger cousin" of AutoResearch on 8x H100 GPUs with his production `nanochat` framework, which suggests the pattern scales beyond toy experiments. The community has already forked it for [macOS/Apple Silicon](https://github.com/miolini/autoresearch-macos) and proposed integrations for older GPUs. The [CPU vs GPU guide](https://www.datacamp.com/blog/cpu-vs-gpu) covers why GPU hardware is non-negotiable for this kind of iterative training.
